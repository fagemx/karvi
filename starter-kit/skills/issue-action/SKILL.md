...
3. **Run quality checks** (read `.claude/project.yaml` for commands):
   - Use `quality.syntax_check` for syntax verification
   - Use `quality.test_command` for running tests
   - Use `quality.lint_command` for linting
   - If project.yaml doesn't exist, fall back to `npm test`

4. **Implement changes following plan.md exactly**:
...
5. **Write and run tests after each change**
...
### Step 8: Create PR and Verify

1. Push branch and create Pull Request

2. If `pr-check` skill is available, run it to monitor and fix CI pipeline
   - pr-check will auto-fix lint/format issues
   - If type or test errors occur, pr-check will exit with details for manual intervention

3. **If checks pass**: Post comment to issue:
   ```bash
...
4. **If checks fail and need manual intervention**: Add "pending" label and exit
