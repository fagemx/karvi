...
### Step 0: Coordination Sync

**Run FIRST before any implementation work.** This prevents multi-agent file conflicts.

```bash
# 1. Claim your scope (use issue number + short label)
edda claim "issue-{id}" --paths "server/relevant-file.js" --paths "server/other-file.js"

# 2. Check active peers and their claims
edda peers

# 3. Check for off-limits files
# If edda injects "Off-limits" in your context, do NOT edit those files.
```

**Rules:**
- Adjust `--paths` to match the files your plan.md will modify
- If a peer already claims files you need, use `edda request "<peer-label>" "I need to modify <file> for issue #{id}"` and wait
